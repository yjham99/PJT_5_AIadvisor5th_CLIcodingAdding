...
try:
    ...
    print("❌ 'google-genai' 라이브러리가 설치되지 않았습니다. 'pip install google-genai'를 실행해주세요.")
    exit(1)
...
class GeminiSmartScheduler:
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("⚠️ GOOGLE_API_KEY가 환경 변수에 설정되지 않았습니다. API 호출 시 실패할 수 있습니다.")
        ...
        self.model_configs = {
            "gemini-2.0-flash": {
                "rpm": 15, "tpm": 1000000, "rpd": 1500, "weight": 1, 
                "desc": "가장 빠르고 효율적 (메인)"
            },
            "gemini-2.0-pro-exp-02-05": { # 최신 실험적 모델
                "rpm": 10, "tpm": 4000000, "rpd": 50, "weight": 2, 
                "desc": "고성능 추론 (서브)"
            },
            "gemini-1.5-flash": {
                "rpm": 15, "tpm": 1000000, "rpd": 1500, "weight": 3,
                "desc": "안정적인 구버전 Flash"
            },
            "gemini-1.5-pro": {
                "rpm": 2, "tpm": 32000, "rpd": 50, "weight": 4,
                "desc": "고성능 (RPM 낮음 주의)"
            }
        }
        ...
        self.priority_list = sorted(
            ...
            key=lambda x: self.model_configs[x]["weight"]
        )
    ...
    def generate_content(self, prompt, system_instruction=None):
        ...
        for model_name in self.priority_list:
            ...
            if stats["calls_today"] >= config["rpd"]:
                logger.info(f"⏭️ {model_name}: 일일 한도 초과 ({stats['calls_today']}/{config['rpd']}). 다음 모델로 넘어갑니다.")
                continue
            ...
            if wait_time > 0:
                if wait_time > 5: # 5초 이상 기다려야 하면 다음 모델로
                    logger.info(f"⏭️ {model_name}: 대기 시간({wait_time:.1f}s) 과다. 다음 모델로 넘어갑니다.")
                    continue
                else:
                    logger.debug(f"⏳ {model_name}: RPM 조절을 위해 {wait_time:.2f}초 대기...")
                    time.sleep(wait_time)

            # 3. API 호출 시도
            try:
                logger.info(f"🚀 {model_name} 호출 중... (오늘 사용: {stats['calls_today']})")
                ...
                if "429" in error_msg or "Resource has been exhausted" in error_msg:
                    logger.warning(f"⚠️ {model_name} 리소스 고갈. 잠시 봉인하고 다음 모델로 전환합니다.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(".env"):
        print("⚠️ 경고: .env 파일이 없습니다. 키가 설정되어 있지 않다면 실패할 것입니다.")
        print("    -> .env.template을 복사하여 .env를 만들고 GOOGLE_API_KEY를 입력하세요.")
    ...
    result = scheduler.generate_content(test_prompt)
    print(f"\n답변:\n{result}")
